/*
  Example: ESP32_I2C_ndef_write_read_restore
  Bus: I2C
  ...
  Goal: Generic NDEF write/read/restore test for standard writable NDEF tags over I2C.
...
void runGenericNdefWriteRestoreTest(rfalNfcDevice *device)
{
  ...
  uint8_t emptyMessagePlaceholder = 0U;
  ...
  Serial.println();
  Serial.println("=== Generic NDEF Write/Read/Restore Test (I2C) ===");
  Serial.print("Type: ");
  ...
  const uint8_t *restorePtr = (originalLen == 0U) ? &emptyMessagePlaceholder : originalMsg;
...
void setup()
{
  ...
  Serial.println("ESP32 I2C generic NDEF write/read/restore test");
